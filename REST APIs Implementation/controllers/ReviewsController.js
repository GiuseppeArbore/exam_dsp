...
module.exports.getSingleReview = function getSingleReview(req, res, next) {
  ...
  Reviews.getSingleReview(req.params.filmId, req.params.reviewerId, userId)
    ...
    Reviews.updateSingleReview(req.body, req.params.filmId, req.params.reviewerId)
      ...
      .catch(function (response) {
        if (response == 403) {
          ...
          utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'Bad request' }], }, 400);
        }
        else if (response == 409) {
          utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The review is completed, you have have not the permission to update ' }], }, 409);
        }
        else {
          utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': response }], }, 500);
        }
