...
/**
 * Retrieve the reviews of the film with ID filmId -> se sei il reviewer, puoi emettere richiesta di edit review o ottenere le richieste che hai fatto per quella review
 * 
 * Input: 
...
exports.getFilmReviews = function (req) {
    ...
    return new Promise((resolve, reject) => {
        ...
        FROM 
            reviews r
        JOIN 
            films f ON r.filmId = f.id
        JOIN 
            (SELECT count(*) total_rows FROM reviews WHERE filmId = ?) c 
        WHERE  
            r.filmId = ?`;
        ...
        db.all(sql, params, (err, rows) => {
            if (err) {
                ...
                let reviews = rows.map((row) => createReview(row, userId));
                resolve(reviews);
...
exports.getSingleReview = function (filmId, reviewerId, userId) {
    ...
    return new Promise((resolve, reject) => {
        ...
        JOIN 
            (SELECT count(*) total_rows FROM reviews WHERE filmId = ?) c 
        WHERE  
            r.filmId = ? AND r.reviewerId = ?`;

        db.all(sql, [filmId, filmId, reviewerId], (err, rows) => {
            if (err)
                reject(err);
            else if (rows.length === 0)
                reject(404);
            else {
                var review = createReview(rows[0], userId);
                resolve(review);
            }
        });
    });
...
exports.deleteSingleReview = function (filmId, reviewerId, owner) {
    return new Promise((resolve, reject) => {
        const sql1 = "SELECT f.owner, r.completed, r.rating FROM films f, reviews r WHERE f.id = r.filmId AND f.id = ? AND r.reviewerId = ?";
        db.all(sql1, [filmId, reviewerId], (err, rows) => {
            ...
            else if (rows[0].completed == 0 && rows[0].rating != null) {
                reject("409")
            }
            else {
...
exports.updateSingleReview = function (review, filmId, reviewerId) {
    return new Promise((resolve, reject) => {
        ...
        db.all(sql1, [filmId, reviewerId], (err, rows) => {
            ...
            else if (rows[0].completed == 1) {  //new code
                reject(409);
            }
            else {
...
const createReview = function (row, userId) {
    var completedReview = (row.completed === 1) ? true : false;
    return new Review(row.fid, row.rid, completedReview, row.reviewDate, row.rating, row.review, userId, row.owner);
}